...
class ConfocalGui(GUIBase):
    ...
    def initMainUI(self):
        ...
        *.ui file and configures the event handling between the modules.
        ...
        if self._scanning_logic.n_channels>2:
            num_of_points_z = (self._scanning_logic.z_range[1] -
                               self._scanning_logic.z_range[0]) / self.slider_res
        ...
        if self._scanning_logic.n_channels>2:
            ...
            self._mw.doubleSpinBox_zslider_min.setValue(self._scanning_logic.z_range[0])
            self._mw.doubleSpinBox_zslider_max.setValue(self._scanning_logic.z_range[1])
        ...
        if self._scanning_logic.n_channels>2:
            ...
            self._mw.doubleSpinBox_zslider_min.setRange(self._scanning_logic.z_range[0],
                                                        self._scanning_logic.z_range[1])
            self._mw.doubleSpinBox_zslider_max.setRange(self._scanning_logic.z_range[0],
                                                        self._scanning_logic.z_range[1])
        ...
        if self.default_meter_prefix:
            ...
            if self._scanning_logic.n_channels>2:
                ...
                self._mw.doubleSpinBox_zslider_min.assumed_unit_prefix = self.default_meter_prefix
                self._mw.doubleSpinBox_zslider_max.assumed_unit_prefix = self.default_meter_prefix
        ...
        # update the z slider
        if self._scanning_logic.n_channels>2:
            self._mw.doubleSpinBox_zslider_min.valueChanged.connect(self.update_slider_z_range)
            self._mw.doubleSpinBox_zslider_max.valueChanged.connect(self.update_slider_z_range)
        ...
        self._mw.depth_channel_ComboBox.addItems(self.cdict.keys())
        self._mw.xy_channel_ComboBox.addItems(self.cdict.keys())
        index_combobox = self._mw.depth_channel_ComboBox.findText("Rainbow")
        self._mw.depth_channel_ComboBox.setCurrentIndex(index_combobox)
        index_combobox = self._mw.xy_channel_ComboBox.findText("Rainbow")
        self._mw.xy_channel_ComboBox.setCurrentIndex(index_combobox)
        
        # Get the colorscale and set the LUTs
        self.my_colors_depth = self.cdict[self._mw.depth_channel_ComboBox.currentText()]()
        self.my_colors_xy = self.cdict[self._mw.xy_channel_ComboBox.currentText()]()
        ...
        self._mw.xy_channel_ComboBox.currentIndexChanged.connect(self.refresh_xy_image)
        self._mw.xy_channel_ComboBox.currentIndexChanged.connect(self.refresh_refocus_image)
        self._mw.depth_channel_ComboBox.currentIndexChanged.connect(self.refresh_depth_image)
        ...
        if self._scanning_logic.n_channels<3:
            self._mw.action_scan_depth_start.setEnabled(False)
            self._mw.action_scan_depth_resume.setEnabled(False)
            self._mw.action_full_range_z.setEnabled(False)
            ...
            self._mw.z_SliderWidget.setEnabled(False)
            self._mw.z_current_InputWidget.setEnabled(False)
            self._mw.actionSave_Depth_Scan.setEnabled(False)
            self._mw.doubleSpinBox_zslider_min.setEnabled(False)
            self._mw.doubleSpinBox_zslider_max.setEnabled(False)
    ...
    def disable_scan_actions(self):
        ...
        self._mw.pushButton_cursor_to_offset.setEnabled(False)
...
#        self._mw.actionRotated_depth_scan.setEnabled(True)
        ...
        self._mw.pushButton_cursor_to_offset.setEnabled(True)
        ...
        if self._scanning_logic.n_channels>2:
            ...
            self._mw.doubleSpinBox_zslider_min.setEnabled(True)
            self._mw.doubleSpinBox_zslider_max.setEnabled(True)
    ...
    def update_from_slider_z(self, sliderValue):
        ...
        """
        minval = self._mw.doubleSpinBox_zslider_min.value()
        z_pos = minval + sliderValue * self.slider_res
        self.update_roi_depth(v=z_pos)
        self.update_input_z(z_pos)
    ...
    def update_slider_z(self, z_pos):
        ...
        """
        minval = self._mw.doubleSpinBox_zslider_min.value()
        self._mw.z_SliderWidget.setValue((z_pos - minval) / self.slider_res)

    def update_slider_z_range(self):
        """ Change the range of the z slider
        """
        minval = self._mw.doubleSpinBox_zslider_min.value()
        maxval = self._mw.doubleSpinBox_zslider_max.value()
        num_of_points_z = (maxval - minval) / self.slider_res
        self._mw.z_SliderWidget.setRange(0, num_of_points_z)
