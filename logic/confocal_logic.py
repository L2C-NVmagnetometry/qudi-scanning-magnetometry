...
class ConfocalHistoryEntry(QtCore.QObject):
    ...
    def __init__(self, confocal):
        ...
        self.n_channels = len(confocal._scanning_device.get_scanner_axes())
...
class ConfocalLogic(GenericLogic):
    ...
    def on_activate(self):
        ...
        self.n_channels = len(self._scanning_device.get_scanner_axes())
...
#            time.sleep(0.01)
        ...
        try:
            if self._scan_counter == 0:
                ...
                if self.n_channels <= 2 :
                    ...
                    if self.n_channels <= 3:
                        start_line = np.vstack([lsx, lsy, lsz][0:self.n_channels])
                    else:
            ...
            if not self._zscan:
                ...
                if self.n_channels > 2:
                    ...
                    image[self._scan_counter, :, 2] = np.zeros(z_shape)
            ...
            if self.n_channels > 2:
                ...
                if self.n_channels <= 3:
                    line = np.vstack([lsx, lsy, lsz][0:self.n_channels])
                else:
                    line = np.vstack(
                        [lsx, lsy, lsz, np.ones(lsx.shape) * self._current_a])
            else:
    ...
    def save_xy_data(self, colorscale_range=None, percentile_range=None):
        ...
        parameters['X offset (m)'] = self.current_x
        parameters['X image range (m)'] = self.image_x_range[1] - self.image_x_range[0]

        parameters['Y offset (m)'] = self.current_y
        parameters['Y image range (m)'] = self.image_y_range[1] - self.image_y_range[0]
    ...
    def save_depth_data(self, colorscale_range=None, percentile_range=None):
        ...
        parameters['Z offset (m)'] = self.current_z
        parameters['Z image range (m)'] = self.image_z_range[1] - self.image_z_range[0]
