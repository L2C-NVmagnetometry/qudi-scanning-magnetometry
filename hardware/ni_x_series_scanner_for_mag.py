...
class NationalInstrumentsXSeriesScanner(Base, ConfocalScannerInterface):
    ...
    def on_activate(self):
        ...
        # In fact this is a problem because we want to be able to move the tip
        # and the sample without closing either the confocal or the scanning module
        # all the time. We will start the task every time we really need it.
    ...
    def get_scanner_axes(self):
        ...
        But since we do not start the analog output task on startup, we might need
        to read the number of channels in the config file.
        """
        possible_channels = ['x', 'y', 'z', 'a']
        if self._scanner_ao_task is None:
            n_ch = len(self._scanner_ao_channels)
            #self.log.warning('Cannot get channel number, analog output task does not exist. Took the number from the config file.')
            #return []
        else:
            n_channels = daq.uInt32()
            daq.DAQmxGetTaskNumChans(self._scanner_ao_task, n_channels)
            n_ch = int(n_channels.value)

        return possible_channels[0:n_ch]
    ...
    def _start_analog_output(self):
        ...
        try:
            ...
            daq.DAQmxCreateTask('ScannerAO{}{}{}'.format(time.localtime().tm_hour, time.localtime().tm_min, time.localtime().tm_sec), daq.byref(self._scanner_ao_task))
    ...
    def set_up_scanner_clock(self, clock_frequency=None, clock_channel=None, idle=False):
        ...
        try:
            ...
            task_name = 'ScannerClock{}{}{}'.format(time.localtime().tm_hour, time.localtime().tm_min, time.localtime().tm_sec)
    ...
    def set_up_scanner(self,
        ...
        try:
            ...
            if len(self._scanner_ai_channels) > 0:
                ...
                daq.DAQmxCreateTask('ScanAnalogIn{}{}{}'.format(time.localtime().tm_hour, time.localtime().tm_min, time.localtime().tm_sec), daq.byref(atask))
